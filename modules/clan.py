...
class Clan(Module):
    ...
    def __init__(self, server):
        ...
        self.commands = {"grci": self.get_random_clan_ids,
                         ...
                         "dcl": self.dismiss,
                         "lccmsg": self.load_clan_chat_message, "dnt": self.donateClan}
    ...
    async def donateClan(self, msg, client):
        clanId = await self.server.redis.get(f"uid:{client.uid}:clan")
        cid = clanId
        if not clanId:
            return
        outcomeGold = int(msg[2]["prc"]["gld"])
        clanCoin = outcomeGold * 3
        await self.server.redis.incrby(f"clans:{cid}:coin", clanCoin)
        await self.server.redis.decrby(f"uid:{client.uid}:gld", outcomeGold)
