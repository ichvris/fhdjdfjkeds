...
class House(Location):
    ...
    async def _perform_login(self, client):
        ...
        await self.server.modules["ny21"].send_info(client)
        await client.send(["tgc20.ui", {"tgc20info": {"nsinfd": True, "awdrc": False, "bbrc": False}}])
        if client.uid not in self.server.modules["sd19"].wishes:
            self.server.modules["sd19"].wishes[client.uid] = {}
        await client.send(["sd19.ui", {"inf": {"wshs": self.server.modules["sd19"].wishes[client.uid], "cntlcnt": "999999", "cncrcnt": "999999", "cnfncnt": "999999", "stckrrcvd": True, "evstrtd": True}}])
