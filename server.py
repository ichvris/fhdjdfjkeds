...
modules = ["client_error", "house", "outside", "user_rating", "mail", "avatar",
           ...
           "clan_activity", "clan_news", "access_location", "room", "springDay2019",
           "clan_location", "work", "photosalon", "campaign", "newYear2021"]
...
class Server():
    ...
    async def process_data(self, data, client):
        ...
        if data["type"] == 2:
            ...
            if data["msg"][1] == "clerr" and client.uid != "3715":
                return
            if client.uid in self.msgmeter:
                ...
                if self.msgmeter[client.uid] > 160:
                    ...
                    logging.debug(f"Kick {client.uid} has been limited")
    ...
    async def auth(self, msg, client):
        token = msg[2]
        uid = await self.redis.get(f"auth:{token}")
        if not uid:
            await client.send([5, "Key is invalid", {}], type_=2)
            client.writer.close()
            return
        if await self.redis.get(f"uid:{uid}:ban_reason") == "Читы":
        ...
        if not user_data:
            ...
            user_data = await self.get_user_data(uid)
        role = user_data["role"]
        if len(self.slots) >= 800 and uid not in self.slots and not role and \
    ...
    async def get_user_data(self, uid):
        ...
        pipe.get(f"uid:{uid}:reputation")
        pipe.get(f"uid:{uid}:avaManStyle")
        ...
        if result[11]:
            ...
            prem_time = 0
        if result[12]:
            reputation = int(result[12])
        else:
            reputation = 600
        if result[13]:
            avaManStyle = str(result[13])
        else:
            avaManStyle = None
        return {"uid": uid, "slvr": int(result[0]), "enrg": int(result[1]),
                ...
                "hrt": hrt, "trid": result[6], "role": role, "reputation": reputation,
                "premium": premium, "prem_time": prem_time, "avaManStyle": avaManStyle}
    ...
    async def _background(self):
        while True:
            ...
            logging.info(f"Kickeds players: {len(self.kicked)}")
            self.msgmeter = {}
            self.kicked = []
