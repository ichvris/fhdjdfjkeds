    ...
    async def room(self, msg, client):
        ...
        if subcommand in ["u", "m", "k", "sa", "sl", "bd", "lks", "hs",
            ...
            if subcommand == "u":
                ...
                acti = await self.server.redis.set(f"uid:{client.uid}:act",
                                    user_data["act"] + 10)
...
async def gen_plr(client, server):
    ...
    chat_bubble = await server.redis.get(f"uid:{uid}:chat_bubble")
...
async def get_city_info(uid, server):
    ...
    ci = {"exp": user_data["exp"], "crt": user_data["crt"],
          ...
          "skid": "iceRinkSkate3", "skrt": int(time.time()+10000000),
