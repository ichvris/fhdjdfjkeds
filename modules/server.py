...
modules = ["client_error", "house", "outside", "user_rating", "mail", "avatar",
           ...
           "craft", "profession", "inventory", "event", "chat_decor"]
...
class Server():
    ...
    def get_user_data(self, uid):
        ...
        pipe.get(f"uid:{uid}:engage_opponent")
        pipe.get(f"uid:{uid}:married_opponent")
        pipe.get(f"uid:{uid}:bdc")
        pipe.get(f"uid:{uid}:tcl")
        result = pipe.execute()
        if not result[0]:
            return None
        if result[7]:
        ...
        if result[9]:
            ...
            role = 0
        if result[11]:
            avaManStyle = str(result[11])
        else:
            avaManStyle = None
        if result[12]:
            engage_opponent = int(result[12])
        else:
            engage_opponent = 0   
        if result[13]:
            married_opponent = int(result[13])
        else:
            married_opponent = 0       
        if result[10]:
            clan_id = int(result[10])
        else:
            clan_id = 0            
        return {"uid": uid, "slvr": int(result[0]), "enrg": int(result[1]),
                "gld": int(result[2]), "exp": int(result[3]),
                "emd": int(result[4]), "lvt": int(result[5]), "crt": crt, "engage_opponent": engage_opponent, "married_opponent": married_opponent,
                "hrt": hrt, "trid": result[6], "role": role, "reputation": int(600), "avaManStyle": avaManStyle, "bdc": result[14], "tcl": result[15]}
